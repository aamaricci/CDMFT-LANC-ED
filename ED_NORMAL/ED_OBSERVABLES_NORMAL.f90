...
contains 
  ...
  subroutine density_matrix_normal()
    ...
    call Bcast_MPI(MpiComm,cluster_density_matrix)
    !
    !
    !
    !
  ...
  subroutine write_dm()
    ...
    if(MpiMaster)then
       ...
       !
       associate(dm => single_particle_density_matrix)
         do ispin=1,Nspin
            unit = free_unit()
            open(unit,file="single_particle_density_matrix_s"//str(ispin)//reg(ed_file_suffix)//".ed")
            do io=1,Nimp
               write(unit,"(*(F20.16,1X))") (dreal(dm(1,1,ispin,ispin,io,jo)),jo=1,Nimp)
            enddo
            if(any(dimag(dm)/=0d0))then
               write(unit,*)
               do io=1,Nimp
                  write(unit,"(*(F20.16,1X))") (dimag(dm(1,1,ispin,ispin,io,jo)),jo=1,Nimp)
               enddo
            endif
            close(unit)
         enddo
