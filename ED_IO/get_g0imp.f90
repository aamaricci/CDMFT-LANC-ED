subroutine ed_get_G0imp_site_n2(self,axis,type,z)
  ...
  L = size(z_)
  !
  call assert_shape(self,[Nspin*Nimp,Nspin*Nimp,L],'ed_get_G0imp','self')
  !
  if(allocated(F))deallocate(F)
  allocate(F(Nambu,Nambu,Nspin,Nspin,Nimp,Nimp,L))  
  F = g0and_bath_function(z_,dmft_bath) 
  !
  select case(type_)
  case default; stop "ed_get_G0imp ERROR: type is neither Normal, nor Anomalous"
  case ('n','N')
...
subroutine ed_get_G0imp_site_n4(self,axis,type,z)
  ...
  !
  L = size(z_)
  call assert_shape(self,[Nspin,Nspin,Nimp,Nimp,L],'ed_get_G0imp','self')
  !
  if(allocated(F))deallocate(F)
  allocate(F(Nambu,Nambu,Nspin,Nspin,Nimp,Nimp,L))  
  F = g0and_bath_function(z_,dmft_bath) 
...
subroutine ed_get_G0imp_site_n6(self,axis,z)
  complex(8),dimension(:,:,:,:,:,:,:),intent(inout) :: self
  character(len=*),optional                         :: axis
  complex(8),dimension(:),optional                  :: z
  character(len=1)                                  :: axis_
  ...
  !
  L = size(z_)
  call assert_shape(self,[Nambu,Nambu,Nspin,Nspin,Nimp,Nimp,L],'ed_get_G0imp','self')
  !
  F = g0and_bath_function(z_,dmft_bath) 
  !
  call deallocate_grids
  !
end subroutine ed_get_G0imp_site_n6
