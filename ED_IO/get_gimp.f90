subroutine ed_get_gimp_site_n2(self,axis,type,z)
  ...
  L = size(z_)
  !
  call assert_shape(self,[Nspin*Nimp,Nspin*Nimp,L],'ed_get_gimp','self')
  !
...
subroutine ed_get_gimp_site_n4(self,axis,type,z)
  ...
  !
  L = size(z_)
  call assert_shape(self,[Nspin,Nspin,Nimp,Nimp,L],'ed_get_gimp','self')
  !
  if(allocated(F))deallocate(F)
  allocate(F(Nambu,Nambu,Nspin,Nspin,Nimp,Nimp,L))  
  F = get_Gimp(z_)
...
subroutine ed_get_gimp_site_n6(self,axis,z)
  complex(8),dimension(:,:,:,:,:,:,:),intent(inout) :: self
  character(len=*),optional                         :: axis
  complex(8),dimension(:),optional                  :: z
  character(len=1)                                  :: axis_
  ...
  !
  L = size(z_)
  call assert_shape(self,[Nambu,Nambu,Nspin,Nspin,Nimp,Nimp,L],'ed_get_gimp','self')
  !
  Self = get_Gimp(z_)
  !
  call deallocate_grids
  !
end subroutine ed_get_gimp_site_n6
