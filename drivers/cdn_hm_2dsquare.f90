program cdn_hm_2dsquare
  ...
  !
  implicit none
  integer                                                                :: Nsym,Nx,Ny,Nso,Nlo,Nlso,iloop,Nb,Nkx,Nky,ilat,jlat,irepl,iw,iii,jjj,kkk,Ntr
  logical                                                                :: converged
  ...
  allocate(Hsym_basis(Nlat,Nlat,Nspin,Nspin,Norb,Norb,Nsym))
  Hsym_basis(:,:,:,:,:,:,1) = lso2nnn(zeye(Nlso)) !Replica onsite energies
  if(Nsym==2)Hsym_basis(:,:,:,:,:,:,2) = abs(lso2nnn(Hloc))  !Replica hopping amplitudes
